...
export default function FlowchartClientPage() {
    ...
    const handleToolCall = useCallback(async (toolName: string, params: any) => {
        ...
        if (toolName === "get_flowchart") {
            const simplified = {
                // biome-ignore lint/suspicious/noExplicitAny: <explanation>
                nodes: currentNodes.map((n: any) => ({ id: n.id, title: n.data.title, label: n.data.label })),
        ...
        if (toolName === "add_node") {
            ...
            if (connectToId) newEdges.push({ id: `e-${connectToId}-${newId}`, source: connectToId, target: newId });
            if (connectFromId) newEdges.push({ id: `e-${newId}-${connectFromId}`, source: newId, target: connectFromId });
        ...
        if (toolName === "update_node") {
            ...
            const newNodes = currentNodes.map(n => {
                if (n.id === nodeId) {
                    return { ...n, data: { ...n.data, title: title || n.data.title, label: label || n.data.label } };
                }
                return n;
            });
        ...
        <div className="w-[calc(100%+47px)] h-[calc(100%+52px)] relative -left-7 -mt-13 bg-background">
            <div className="absolute top-4 left-4 z-10 p-2 flex items-center gap-4">
                ...
                <div className="w-[300px]">
                    <ConversationBar
                        agentId={process.env.NEXT_PUBLIC_ELEVENLABS_FLOWCHART_AGENT_ID || ""}
